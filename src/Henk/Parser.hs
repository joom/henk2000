...
expr =
  ...
  choice
    [ (<?> "pi expression") $ do
        _ <-
          symbol henkT "|~|" <|> symbol henkT "∏" <|> symbol henkT "𝚷" <|>
          symbol henkT "\\/" <|>
          symbol henkT "∀"
    ...
    , (<?> "lambda expression") $ do
        _ <-
          symbol henkT "\\" <|> symbol henkT "/\\" <|> symbol henkT "λ" <|>
          symbol henkT "Λ"
  ...
  choice
    ...
    , (<?> "sort expression") $
      ...
      choice
        ...
        , do _ <- symbol henkT "*"
             return Star
        , do _ <- symbol henkT "||" <|> symbol henkT "□"
             return Box
        ]
    , do _ <- symbol henkT "?"
         return Unknown
