...
----------------------------------------------------------------
program :: Program -> Doc
program (Program tds vds) = vsep (map tDecl tds) $$ vsep (map vDecl vds)
...
tDecl :: TDecl -> Doc
tDecl (TDecl tv tvs) =
  text "data" <+> bindVar tv $$ indent (text "=" <+> br_list (map bindVar tvs))
...
----------------------------------------------------------------
expr :: Expr -> Doc
expr (LamExpr (TVar (Var v) (SortExpr Star)) ex1) =
  sep [text "Λ" <> text v <> text ".", expr ex1]
expr (LamExpr tv ex1) = sep [text "λ" <> bindVar tv <> text ".", expr ex1]
expr (PiExpr (TVar Anonymous ex1) ex2) =
  sep [left_parents_function ex1 (expr ex1), text "->", (expr ex2)]
expr (PiExpr (TVar (Var v) (SortExpr Star)) ex2) =
  sep [text "∀" <> text v <> text ".", expr ex2]
...
expr (CaseExpr ex1 as _) =
  text "case" <+> expr ex1 <+> text "of" <+> indent (br_list (map alt as)) -- $$  text ":" <+> expr ex
...
alt (Alt tc tcas dcas ex) =
  ...
  comma_sep (map boundVar dcas) <+> text "=>" <+> expr ex
...
tryListLiteral (AppExpr (AppExpr (AppExpr (VarExpr (TVar (Var "Cons") _)) _) el) rest) =
  (el :) `fmap` tryListLiteral rest
